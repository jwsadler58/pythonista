...
class TTT:
	...
	def train(self, n=100):
		...
		print("Entries: {0:4} Win: {1:2} Loss: {2:2} Draw: {3:2}".format(*self.pwld))
		self.stats.append(self.pwld)
	...
	def update_policy(self):
		...
		# these values determined experimentally to converge "quickly" in 2k-3k games
		alpha = 0.9		# relative weight of new information in most recent move
		beta  = 0.5		# decay rate: 1.0 means no decay
		...
		for i in range(8):
			...
			for i in range(len(moves)-1, -1, -1):
				...
				m = moves[i]
				r = self.policy[bd]
				if reward > 0:		# augment winning move closer to 1.0
					r[m] += (1 - r[m]) * alpha
				else:				# diminish losing move closer to 0.0
					r[m] -= r[m] * alpha
...
class MyScene (Scene):
	def setup(self):
		...
		self.policy_label.text = 'Entries: ' + str(self.ttt.pwld)
